...
class TestServerWebSocket:
    ...
    @pytest.mark.asyncio
    async def test_continuous_session_protocol(self, server):
        """Continuous mode exposes effective turn configuration."""
        import websockets

        ws_url, _ = server
        async with websockets.connect(
            f"{ws_url}?session_id=continuous-test&client_id=continuous-client"
        ) as ws:
            info = await recv_until_type(ws, "session_info")
            ...
            started = await recv_until_type(ws, "session_started")
            assert started["mode"] == "continuous"
            assert started["config"]["semantic_enabled"] is False
            await recv_until_type(ws, "listening_started")

    @pytest.mark.asyncio
    async def test_manual_interrupt_protocol(self, server):
        """Manual interruption always flushes client playback."""
        import websockets

        ws_url, _ = server
        async with websockets.connect(ws_url) as ws:
            await recv_until_type(ws, "session_info")
            await ws.send(json.dumps({"type": "interrupt"}))
            cancelled = await recv_until_type(ws, "tts_cancelled")
            assert cancelled["type"] == "tts_cancelled"

    @pytest.mark.asyncio
    async def test_client_session_resume(self, server):
        """A stable client_id resumes metadata without replacing session_id."""
        import websockets

        ws_url, _ = server
        url = f"{ws_url}?session_id=resume-session&client_id=resume-client"
        async with websockets.connect(url) as ws:
            first = await recv_until_type(ws, "session_info")
            ...
            await recv_until_type(ws, "session_started")

        await asyncio.sleep(0.1)
        async with websockets.connect(url) as ws:
            resumed = await recv_until_type(ws, "session_info")
            assert resumed["resumed"] is True
            assert resumed["sessionId"] == "resume-session"
